...
function imgevr_upload_image(){
    ...
    if ( empty($mime) ) {
        if ( !empty( $_POST['file'] ) && preg_match('/image\/[a-z0-9]+/', $_POST['file'], $matches) ) {
            ...
            factory_fr110_json_error('Unable to get mime type of the file.');
        }
    }
    ...
    $imageName = ( !empty($name) && $name !== 'undefined' ) 
                    ? factory_fr110_filename_without_ext($name) 
                    : 'img_' . uniqid();
    ...
    if ( isset( $_FILES['file'] ) ) {
        ...
        if ( empty( $_FILES['file']['size'] ) ) {
            factory_fr110_json_error('Sorry, the error of reading image data occured. May be the image is empty of has incorrect format.');
        }
        ...
        if ( preg_match('/base64,(.*)/', $_POST['file'], $matches) ) {
            ...
            if ( !$success ) factory_fr110_json_error('Unable to save the image.');
        } else {
            factory_fr110_json_error('Incorrect file format (base64).');
        }
    }
